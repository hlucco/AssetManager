  ...
  MongoClient.connect(
    ...
    async (error: Error, client: MongoClient.MongoClient) => {
      if (error === null) {
        ...
        let existing: AssetClass = await collectionAssetClasses.findOne({
    ...
    async (error: Error, client: MongoClient.MongoClient) => {
      if (error === null) {
        ...
        if (existing === null) {
          ...
          let updatedAccount = existing.accounts.find(
    ...
    async (error: Error, client: MongoClient.MongoClient) => {
      if (error === null) {
        ...
        if (existing === null) {
          ...
          let accountToDelete: PortfolioAccount = existing.accounts.find(
            (i) => i.id === req.params.accountId
          );
          let updatedAccounts: PortfolioAccount[] = existing.accounts.filter(
            (i) => i.id !== req.params.accountId
          );
          ...
          await collectionAssetClasses.updateOne(
            { id: assetId },
            { $set: { accounts: updatedAccounts, totalValue: newValue } }
          );
